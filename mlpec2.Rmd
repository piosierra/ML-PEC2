...
`r if(sum(complete.cases(oh_data)) == nrow(oh_data)){"Comprobamos que no hay obervaciones con datos faltantes."}``r if(sum(complete.cases(oh_data)) != nrow(oh_data)){paste("El archivo de datos tiene",nrow(oh_data)- sum(complete.cases(oh_data)), "observaciones a en las que faltan datos", sep=" ")}`
